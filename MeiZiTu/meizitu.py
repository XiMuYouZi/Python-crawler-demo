...
import os
from MeiZiTu.DownLoad import request ##导入模块变了一下
...
class mzitu():
    ...
    def all_url(self, url):
        ...
        for a in all_a:
            ...
            self.mkdir(path)
            os.chdir("/Users/Chan/Downloads/meizitu/"+path)
            href = a['href']
            self.html(href)
    ...
    def mkdir(self, path):
        path = path.strip()
        isExists = os.path.exists(os.path.join("/Users/Chan/Downloads/meizitu/", path))
        if not isExists:
            print(u'建了一个名字叫做', path, u'的文件夹！')
            os.makedirs(os.path.join("/Users/Chan/Downloads/meizitu/", path))
            return True
        else:
